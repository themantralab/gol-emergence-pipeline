...
- **FUTURE EXPANSION** — not used now; retained because it enables planned work (stratified sampling, the explorer model)
- **EXTERNAL DIAGNOSIS** — not used by the model at all; retained to evaluate/verify model behaviour from the outside

> The new world model's *only* direct input is the seed set. Trajectories are produced by simulating seeds through the GoL engine at training time. Everything else below is either future-facing or diagnostic.
...
|------|-------------|------|
| `seeds.npy` | (1.5M, 16, 16) uint8 · 367M | The seed corpus. Embedded and simulated to produce training trajectories. The sole direct model input. |
...
|------|-------------|--------------|
| `lifespans.npy` | (1.5M,) · 5.8M | Per-seed lifespan. Enables stratified sampling toward long-lived patterns (spec open-question #4). Not used until/unless we enable stratification. |
| `buckets.npy` | (1.5M,) · 5.8M | Density-band bucket assignment per seed (4 bands: 0.03–0.08, 0.08–0.15, 0.15–0.22, 0.22–0.3). Stratification support. |
